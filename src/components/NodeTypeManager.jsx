...
const NodeTypeManager = () => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md p-4 animate-in fade-in duration-300">
      <div className="flex flex-col w-full max-w-4xl max-h-[85vh] rounded-3xl bg-white shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">

        {/* Header */}
        <div className="flex-none px-8 py-6 border-b border-gray-100 flex justify-between items-center bg-white z-10">
          <div>
            <h2 className="text-xl font-black text-gray-900 tracking-tight">实体类型配置</h2>
            <p className="text-sm text-gray-400 mt-1">管理图谱实体类型定义、样式与自动识别规则</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/30 p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Object.entries(nodeTypeConfigs).map(([key, config]) => {
              ...
              return (
                <div key={key} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-5 hover:shadow-xl hover:scale-[1.01] transition-all duration-300">
                  {/* Top Row: Identity */}
                  <div className="flex items-start gap-3">
                    ...
                    {/* Icon & Delete */}
                    <div className="bg-slate-50 rounded-xl p-1.5 flex items-center gap-1.5 border border-slate-100">
                      <input
                        className="w-10 text-center bg-transparent border-none p-0 text-xl focus:ring-0"
                        value={config.icon}
                      ...
                      <button
                        ...
                        className={`w-10 h-10 flex items-center justify-center rounded-lg hover:bg-white transition-all ${nodeTypeUsage[key] ? 'text-gray-200 cursor-not-allowed' : 'text-gray-400 hover:text-red-500 hover:shadow-sm'
                          }`}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {commonColors.map((color) => (
                        <button
                          key={color}
                          type="button"
                          className={`h-6 w-6 rounded-full border-2 transition ${config.color === color ? 'border-gray-900' : 'border-white'
                            }`}
